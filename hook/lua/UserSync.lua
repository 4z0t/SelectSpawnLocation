...
function OnSync()
    ...
    if Sync.Teamkill and not SessionIsReplay() then
        ...
        GpgNetSend('TeamkillHappened', data.time, data.victim.id, data.victim.name, data.instigator.id,
            data.instigator.name)
    ...
    if Sync.Markers then
        import('/mods/DynamicSpawns/modules/Markers.lua').UpdateMarkers(Sync.Markers)
    end

    if Sync.DeleteMarkers then
        import('/mods/DynamicSpawns/modules/Markers.lua').Delete()
    end
